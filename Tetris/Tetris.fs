...
module Game =
    ...
    let update msg state =
                ...
                if state.tetrimino <> nxt then
                    ...
                    |> function
                        | true ->
                            ...
                            let res =
                                state.grid.board |> TetrisBoard.setTetrimino nxt
        ...
        | RotL ->
            { state with
                tetrimino =
                    state.tetrimino |> Tetrimino.rotateLeft state.grid.board
                lastUpdated =
                    if
                        state.tetrimino.shape = Shape.O
                        || state.tetrimino
                           |> Tetrimino.rotateLeft state.grid.board = state.tetrimino
                        || state.tetrimino
        ...
        | RotR ->
            { state with
                tetrimino =
                    state.tetrimino |> Tetrimino.rotateRight state.grid.board
                lastUpdated =
                    if
                        state.tetrimino.shape = Shape.O
                        || state.tetrimino
                           |> Tetrimino.rotateRight state.grid.board = state.tetrimino
                        || state.tetrimino
        ...
        | Left ->
            { state with
                tetrimino =
                    state.tetrimino |> Tetrimino.moveLeft state.grid.board
                lastUpdated =
                    if
                        state.tetrimino.shape = Shape.O
                        || state.tetrimino
                           |> Tetrimino.moveDown state.grid.board
                           <> state.tetrimino
                        || state.tetrimino
                           |> Tetrimino.moveLeft state.grid.board = state.tetrimino
                    then
                        state.lastUpdated
                    else
                        DateTime.Now }
        | Right ->
            { state with
                tetrimino =
                    state.tetrimino |> Tetrimino.moveRight state.grid.board
                lastUpdated =
                    if
                        state.tetrimino.shape = Shape.O
                        || state.tetrimino
                           ...
                           <> state.tetrimino
                        || state.tetrimino
                           |> Tetrimino.moveRight state.grid.board = state.tetrimino
                    then
                        state.lastUpdated
                    else
                        DateTime.Now }
        ...
        let toColor state =
            ...
            [| for y in 3 .. (Array2D.length1 cells) - 1 - 1 do
                   for x in 2 .. (Array2D.length2 cells) - 1 - 2 do
                       if
                           not state.isOver && minoPos |> Array.contains (x, y)
                       then
