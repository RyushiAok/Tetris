...
module Game = 
    ...
    let update msg state =  
        ...
        | Update -> 
            ...
            then state  
                ...
                if state.tetrimino <> nxt then  
                    ...
                    |> function 
                        | true ->  
                            let existsOtherBlock = nxt |> Tetrimino.existsOtherBlock state.grid.board 
                            let res = state.grid.board |> TetrisBoard.setTetrimino nxt  
                            { state with 
                                isOver = true
                                grid = 
                                    { state.grid with 
                                        board = if existsOtherBlock then state.grid.board else res.newBoard
                                    }
                            }  
                        | false -> 
        ...
        | RotL     -> 
            { state with 
                ...
                lastUpdated =
                    if state.tetrimino.shape = Shape.O 
        ...
        | Left   -> 
            { state with 
                ...
                lastUpdated = 
                    if state.tetrimino.shape = Shape.O 
                       || state.tetrimino |> Tetrimino.moveDown state.grid.board <> state.tetrimino
    ...
    let shapeToColor shape = 
        match shape with
        | Shape.I -> "#00ffff" | Shape.O -> "#ffd700" | Shape.J -> "#1e90ff"
        | Shape.L -> "#ffa500" | Shape.S -> "#00FF00" | Shape.Z -> "#ff0000"
        | Shape.T -> "#800080"       
    ...
    let boardView state dispatch =
        ...
        let toColor state =
            ...
            let minoPos = state.tetrimino.pos |> Array.map(fun (x,y) -> (x+state.tetrimino.x,y+state.tetrimino.y))
                ...
                for y in 3..(Array2D.length1 cells) - 1-1 do 
                    for x in 2..(Array2D.length2 cells) - 1-2 do 
                        if not state.isOver && minoPos |> Array.contains (x,y) then
                            ...
                            | Square.Empty -> yield "#222222"
                            | Square.Guard -> yield "#AAAAAA" 
                            | Square.Mino shape -> yield shapeToColor shape
            |]
            ...
            StackPanel.children [ 
                UniformGrid.create [ 
                    ...
                    UniformGrid.children ( 
                        state.hold
                        |> Option.map(fun mino ->
                            { mino with pos = mino.pos |> Array.map(fun (a,b) -> (a+1,b+1)) } )
                        |> Option.map (fun mino ->
                            [|
                                for y in 0..3 do 
                                    for x in 0..3 do 
