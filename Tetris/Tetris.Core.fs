...
module TetrisBoard =
    ...
    let init =  
        Array2D.init 24 16 (fun y x -> 
            if (3 <= x && x <= 12 && 0 <= y && y <= 21) then Square.Empty else Square.Guard ) 
        ...
        let dif = List.rev [
            ...
            for y in 21..(-1)..2 do 
                ...
                if isLineFilled then
                    for x in 3..12 do nxt.[y,x] <- Empty   
                    yield y
                    dy <- dy+1
                else 
                    yield y+dy 
...
module Tetrimino = 
    ...
    let initMino = function
        | I -> { x = 7; y = 2; shape = Shape.I; pos = [| (0,0);(-1,0);( 1, 0);( 2, 0) |]; } 
        | O -> { x = 7; y = 2; shape = Shape.O; pos = [| (0,0);( 1,0);( 0,-1);( 1,-1) |]; }  
        | S -> { x = 7; y = 2; shape = Shape.S; pos = [| (0,0);(-1,0);( 0,-1);( 1,-1) |]; } 
        | Z -> { x = 7; y = 2; shape = Shape.Z; pos = [| (0,0);(-1,-1);( 0,-1);( 1,0) |]; }  
        | J -> { x = 7; y = 2; shape = Shape.J; pos = [| (0,0);( 1,0);(-1, 0);(-1,-1) |]; }  
        | L -> { x = 7; y = 2; shape = Shape.L; pos = [| (0,0);( 1,0);(-1, 0);( 1,-1) |]; }  
        | T -> { x = 7; y = 2; shape = Shape.T; pos = [| (0,0);(-1,0);( 1, 0);( 0, 1) |]; }  
    ...
    let rotateRight board mino =  // https://tetrisch.github.io/main/spins.html
        if mino.shape = Shape.O then mino else 
        let pos = [| for (dx,dy) in mino.pos do  (-dy, dx) |]  
        let r_asixs = 
            ...
            | (mx,my)::t ->
                let rec loop2 = function 
                    ...
                    | (hx,hy)::t2 -> 
                        ...
                        |> function | true  -> loop2 t2 
                                    | false -> { mino with x = nx; y = ny; pos = pos;} 
                loop2  r_asixs  
        let moved = loop [(0,0);(0,1);(-1,0);(1,0);(0,-1);(2,0);(0,-2);(-2,0);(0,2);]
        let modify =  // 高さの調整
            ...
            | (mx,my)::t ->
                let rec loop2 = function
                    ...
                    | (hx,hy)::t2 -> 
                        ...
                        pos  
                        |> Array.exists(fun (dx,dy) -> board |> TetrisBoard.isFilled (nx+dx) (ny+dy) )
                        |> function | false -> { mino with x = nx; y = ny; pos = pos;}   
                                    | true -> loop2 t2 
                loop2  r_asixs
    ...
    let generate = 
        ...
        let blocks = [|
            initMino Shape.I; initMino Shape.O; initMino Shape.S;  
            initMino Shape.Z; initMino Shape.J; initMino Shape.L; 
            initMino Shape.T; 
        |] 
        fun reset -> 
            if reset then queue <- []
